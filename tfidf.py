...
def tf(word, blob):#word means the single keyword; blob means the document vector
...
#tb() is equal to TextBlob("sentence string")
...
for i, blob in enumerate(bloblist):
    ...
    scores = {word: tfidf(word, blob, bloblist) for word in blob.words}#dictionary of word and its tf-idf score
    sorted_words = sorted(scores.items(), key=lambda x: x[1], reverse=True)
    for word, score in sorted_words[:3]:#print出前三名word and its score
